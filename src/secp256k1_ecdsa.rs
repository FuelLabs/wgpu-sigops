...
pub async fn ecrecover(
    ...
    for _ in num_signatures..next_pow_2 {
        all_sig_bytes.extend(signatures[0].as_slice());
        all_msg_bytes.extend(messages[0].as_slice());
    }

    // TODO: fix the impl to use 0s instead
    ...
    let result_buf = create_empty_sb(&device, (16 * next_pow_2 * std::mem::size_of::<u32>()) as u64);
