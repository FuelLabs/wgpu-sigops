...
@group(0) @binding(0) var<storage, read_write> r: array<u32>;
@group(0) @binding(1) var<storage, read_write> s: array<u32>;
...
fn secp256k1_recover_1(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    for (var i = 0u; i < 32u; i ++) {
        r_bytes_be[id * 32u + i] = r[i];
        s_bytes_be[id * 32u + i] = s[i];
        msg_bytes_be[id * 32u + i] = msg[i];
    }
