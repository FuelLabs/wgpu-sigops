...
pub async fn verify() {
    ...
    for log_limb_size in 13..14 {
        for _ in 0..10 {
            ...
            do_eddsa_test(&verifying_key, &signature, &message, log_limb_size).await;
        }
    }
}
...
pub async fn do_eddsa_test(
    ...
    let pk_u32s: Vec<u32> = bytemuck::cast_slice(&pk_bytes_be).to_vec();
    let msg_u32s: Vec<u32> = bytemuck::cast_slice(message).to_vec();
