...
pub async fn test_secp256k1_glv() {
    ...
    let half_scalar_p = &scalar_p / BigUint::from(2u32);
    let h = Fr::from(half_scalar_p);
    ...
    let b1 = &scalar_p - BigUint::parse_bytes(b"e4437ed6010e88286f547fa90abfe4c3", 16).unwrap();
    let a2 = BigUint::parse_bytes(b"114ca50f7a8e2f3f657c1108d9d44cfd8", 16).unwrap();
    let b2 = BigUint::parse_bytes(b"3086d221a7d46bcde86c90e49284eb15", 16).unwrap();

    let mut rng = ChaCha8Rng::seed_from_u64(2);

    for _ in 0..1000 {
        // Generate a random scalar k
        let k: BigUint = rng.sample::<BigUint, RandomBits>(RandomBits::new(256)) % &scalar_p;

        // Split k into k1 and k2
        let c1 = (&b2 * &k) / &scalar_p;
        let c2 = ((&scalar_p - &b1) * &k) / &scalar_p;

        let mut k1 = Fr::from(&k - &c1 * &a1 - &c2 * &a2);
        let mut k2 = Fr::from((&scalar_p - &c1) * &b1 - &c2 * &b2);

        // Generate a random point
        let r: BigUint = rng.sample::<BigUint, RandomBits>(RandomBits::new(256)) % &scalar_p;
